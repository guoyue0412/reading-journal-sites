...
export default async function Home() {
  ...
  return (
    <ResearchShell>
      ...
      <section className="research-feature"><header><p>NOTES AND ANALYSIS</p><h2>Recent writing</h2><Link href="/index">Full index →</Link></header><div className="research-list research-list--titles">{recentEntries.map((entry) => <article key={entry.slug}><p>{entry.date}</p><h3><Link href={`/blog/${entry.slug}`}>{entry.title}</Link></h3></article>)}</div></section>
      <ContentOverview entries={entries} />
    </ResearchShell>
  );
}
