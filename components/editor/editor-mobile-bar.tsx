...
export function EditorMobileBar({ pane, saveState, disabled, onAdd, onPaneChange, onPublish, onImport, onExport }: {
  pane: MobilePane;
  saveState: SaveState;
  disabled: boolean;
  onAdd: () => void;
  ...
  return <nav className="studio-mobile-bar" aria-label="移动端编辑操作">
    ...
    <details className="studio-mobile-tools">
      <summary>更多工具</summary>
      <div>
        <button type="button" onClick={onImport}>导入 Markdown</button>
        <button type="button" disabled={disabled} onClick={onExport}>导出 Markdown</button>
      </div>
    </details>
  </nav>;
}
