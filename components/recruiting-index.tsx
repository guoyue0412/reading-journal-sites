...
export function RecruitingIndex({ entries }: { entries: ContentEntry[] }) {
  ...
  return (
    <>
      <section className="archive-recruiting-stages" aria-label="秋招阶段筛选">
      ...
      <div className="archive-recruiting-ledger" aria-live="polite">
        {!entries.length ? (
          <p className="empty-state">还没有秋招岗位档案。前往<Link href="/editor">编辑器创建第一篇 Markdown</Link>。</p>
        ) : filteredEntries.length ? filteredEntries.map((entry) => (
          <article className="archive-recruiting-entry" key={entry.slug}>
            <div className="archive-recruiting-entry__stage"><span>当前阶段</span><strong>{stageLabels[entry.applicationStage ?? "applied"]}</strong></div>
            <div className="archive-recruiting-entry__body">
              ...
              <p>{entry.summary}</p>
              {entry.nextAction ? <p className="archive-recruiting-entry__next"><span>下一步</span>{entry.nextAction}</p> : null}
            </div>
            <Link className="archive-recruiting-entry__link" href={`/post/${entry.slug}`} aria-label={`查看${entry.company}${entry.role}档案`}>→</Link>
          </article>
        )) : (
