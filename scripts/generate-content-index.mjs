...
const legacyOutputPath = cliPath("--legacy-output", path.join(root, "lib/content/legacy-generated.ts"));
...
async function collectEntries() {
  ...
  return {
    published: publishedEntries.map(({ entry }) => runtimeEntry(entry)),
    all: entries.map(({ entry }) => runtimeEntry(entry)),
  };
}

const entries = await collectEntries();
const generated = `// Generated by scripts/generate-content-index.mjs. Do not edit.\nimport type { ContentEntry } from "./types";\n\nexport const CONTENT_ENTRIES: ContentEntry[] = ${JSON.stringify(entries.published, null, 2)};\n`;
const legacyGenerated = `// Generated by scripts/generate-content-index.mjs. Do not edit.\nimport type { ContentEntry } from "./types";\n\nexport const LEGACY_CONTENT_ENTRIES: ContentEntry[] = ${JSON.stringify(entries.all, null, 2)};\n`;
await fs.mkdir(path.dirname(outputPath), { recursive: true });
await fs.writeFile(outputPath, generated, "utf8");
await fs.mkdir(path.dirname(legacyOutputPath), { recursive: true });
await fs.writeFile(legacyOutputPath, legacyGenerated, "utf8");
console.log(`Generated ${entries.published.length} public and ${entries.all.length} legacy content entries.`);
